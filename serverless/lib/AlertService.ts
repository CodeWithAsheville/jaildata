/**
 * AlertService - Centralized error monitoring and notification service for JailData
 *
...
// DynamoDB table name for error deduplication (using existing error cache table)
const ERROR_CACHE_TABLE =
    process.env.ERROR_CACHE_TABLE ||
    `jaildata-error-cache-${process.env.STAGE || "dev"}`;
        ...
        if (isNewError) {
            ...
            const result = await dynamodb.send(
                new UpdateItemCommand({
                    ...
                    UpdateExpression:
                        "ADD errorCount :inc SET lastSeen = :now, #ttl = :ttl",
            ...
            return {
                ...
                firstSeen: parseInt(
                    result.Attributes?.firstSeen?.N || now.toString()
                ),
                lastSeen: parseInt(
                    result.Attributes?.lastSeen?.N || now.toString()
                ),
                lastReported: parseInt(
                    result.Attributes?.lastReported?.N || "0"
                ),
            };
        }
    } catch (error) {
 ...
 */
async function updateLastReported(
    errorKey: string,
    timestamp: number
): Promise<void> {
    try {
        ...
        console.warn(
            "Failed to update last reported timestamp in DynamoDB:",
            error
        );
    }
}
...
async function publishMetric(
    ...
    try {
        const command = new PutMetricDataCommand({
            Namespace: `JailData/${process.env.STAGE || "dev"}`,
            MetricData: [
                {
                    MetricName: name,
                    Value: value,
...
async function sendAlert(
    ...
    try {
        ...
        const prefix = `[JailData ${environmentInfo.stage}] ${severity} ${category}: `;
...
const AlertService = {
    ...
    async logError(
        ...
        const exceedsThreshold =
            cacheEntry.errorCount >= ERROR_REPORT_THRESHOLD;
        const exceedsTimeInterval =
            now - cacheEntry.lastReported > ERROR_REPORT_INTERVAL_MS;
        ...
        if (
            ...
            (severity === Severity.ERROR &&
                (exceedsThreshold || exceedsTimeInterval)) ||
            (severity === Severity.WARNING &&
                cacheEntry.errorCount >= ERROR_REPORT_THRESHOLD * 2)
        ) {
            await sendAlert(
                severity,
                category,
